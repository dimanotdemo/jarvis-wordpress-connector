...
| `POST` | `/wp-json/jarvis/v1/plugin`   | Manage a plugin (`activate`/`deactivate`/`delete`, or `install` from wp.org by slug) |
| `POST` | `/wp-json/jarvis/v1/theme`    | Manage a theme (`activate`/`delete`) |
| `GET`  | `/wp-json/jarvis/v1/integrity` | Core file checksum check (modified/missing core files) |

`/state` also reports `environment` (PHP/MySQL/cron) and `security` flags, which JARVIS turns into severity-ranked findings.
