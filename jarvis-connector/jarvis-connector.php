...
if ( ! defined( 'JARVIS_CONNECTOR_VERSION' ) ) {
	define( 'JARVIS_CONNECTOR_VERSION', '1.9.0' );
}
 ...
 */
function jarvis_connector_state( WP_REST_Request $request = null ) {
	if ( ! function_exists( 'get_plugins' ) ) {
		require_once ABSPATH . 'wp-admin/includes/plugin.php';
	}
	require_once ABSPATH . 'wp-admin/includes/update.php';

	// JARVIS can force a fresh self-update check by passing ?refresh=connector
	// (set by the daily sweep + an explicit "Sync now"). Clearing the cached
	// GitHub release result here means the next wp_update_plugins() below
	// re-fetches the latest connector version immediately, instead of waiting
	// out the cache — so connector updates surface on demand, not hours later.
	if ( $request && 'connector' === (string) $request->get_param( 'refresh' ) ) {
		delete_site_transient( 'jarvis_connector_latest_release' );
	}
...
function jarvis_connector_environment() {
	...
	return array(
		...
		// Origin host facts — the connector runs ON the box, so it sees past any
		// CDN (Cloudflare) that masks the public IP. JARVIS maps these to a host.
		'server_addr'     => isset( $_SERVER['SERVER_ADDR'] ) ? $_SERVER['SERVER_ADDR'] : null,
		'server_hostname' => function_exists( 'gethostname' ) ? ( gethostname() ?: null ) : null,
...
function jarvis_connector_latest_release( $force = false ) {
	...
	// Cache success for 1h (GitHub's unauthenticated rate limit is 60/hr per IP;
	// update checks only run on a transient refresh, ~hourly), a miss for 30min.
	// JARVIS can also force an immediate re-check via /state?refresh=connector.
	set_site_transient( $cache_key, $result ? $result : array( 'miss' => true ), $result ? HOUR_IN_SECONDS : 30 * MINUTE_IN_SECONDS );
	return $result;
}
 ...
 *
 *  CRITICAL: only rename when the extracted folder name actually differs. The
 *  release asset zip already extracts to "jarvis-connector/", and moving a
 *  folder ONTO ITSELF with overwrite deletes the destination (= the source)
 *  first, destroying the extracted files → "Filesystem error" + a half-broken
 *  install. That bug (≤1.7.0) is exactly what stranded the connector on update. */
function jarvis_connector_fix_update_folder( $source, $remote_source, $upgrader, $hook_extra = array() ) {
	...
	// Already the canonical folder (the named release asset) — nothing to do.
	if ( 'jarvis-connector' === basename( untrailingslashit( $source ) ) ) {
		return $source;
	}
...
/** Opt the connector into WordPress's NATIVE plugin auto-update (the wp-cron
 *  updater that keeps other plugins current). Combined with the self-update
 *  hook above, WP detects a new GitHub release and applies it automatically,
 *  safely, in its own background context — no JARVIS involvement, no re-upload.
 *  This is the right "auto-update inside WP" mechanism. */
