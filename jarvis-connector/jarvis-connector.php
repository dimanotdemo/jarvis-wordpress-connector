...
if ( ! defined( 'JARVIS_CONNECTOR_VERSION' ) ) {
	define( 'JARVIS_CONNECTOR_VERSION', '1.3.0' );
}
...
function jarvis_connector_upgrade_response( $type, $target, $prev_version, $result, $skin, WP_REST_Request $request ) {
	...
	if ( false === $result || null === $result ) {
		return new WP_REST_Response( array(
			'ok' => true, 'status' => 'already_current', 'type' => $type, 'target' => $target,
		), 200 );
	}
	...
	if ( 'plugin' === $type ) {
		$plugins     = get_plugins();
		$new_version = isset( $plugins[ $target ]['Version'] ) ? $plugins[ $target ]['Version'] : null;
	} elseif ( 'theme' === $type ) {
		$theme       = wp_get_theme( $target );
		$new_version = $theme->exists() ? $theme->get( 'Version' ) : null;
	} elseif ( 'core' === $type ) {
